...
# np.random.seed(42) # to use for validation


 # -----------------------------------------------------------------------------------------------------------------------------------------#

# EDSS is a dictionary 
...
def optimization(veg_1_edss, veg_2_edss, total_workers_available, total_demand, land_veg_1 = None, land_veg_2=None, sowed=None, original_1=0, original_2=0, too_much=False):
    ...
    # This is set of constraints for when the land has already been booked and sowed paartilly and there is space to satisfy further demand
        ...
        c2 = model_1.addConstr ( veg_1  >= veg_1_edss['demand'],  name = "c2" )    
        c3 = model_1.addConstr (veg_2 >= veg_2_edss['demand'], name = "c3" )
        c4 = model_1.addConstr ( veg_1  >= original_1,  name = "c4" )    
        c5 = model_1.addConstr (veg_2 >= original_2, name = "c5" )
        ...
        #  because land is function of seeds i.e. func(seeds) = land
    ...
    if too_much == False and land_veg_1 == None and land_veg_2==None:
        ...
        c2 = model_2.addConstr ( veg_1  >= veg_1_edss['demand'],  name = "c2" )    
        c3 = model_2.addConstr (veg_2 >= veg_2_edss['demand'], name = "c3" )
        c4 = model_2.addConstr ( veg_1  >= original_1,  name = "c4" )    
        c5 = model_2.addConstr (veg_2 >= original_2, name = "c5" )
    ...
    Q = goal_programming_model.addVar ( name = " Q", vtype = GRB.CONTINUOUS) # Q is unconstrained to allow at least weak Pareto solutions to be certainly found 
    ...
    if too_much == False and land_veg_1 == None and land_veg_2 == None:
        ...
        c2 = goal_programming_model.addConstr ( veg_1  >= veg_1_edss['demand'],  name = "c2" )    
        c3 = goal_programming_model.addConstr (veg_2 >= veg_2_edss['demand'], name = "c3" )
        c4 = goal_programming_model.addConstr ( veg_1  >= original_1,  name = "c4" )    
        c5 = goal_programming_model.addConstr (veg_2 >= original_2, name = "c5" )
    ...
    if too_much == True:
        if veg_1_edss['demand'] > veg_1_results [2]:
            veg_1_edss['demand'] = veg_1_results [2]
        if veg_2_edss['demand'] > veg_2_results [2]:
            veg_2_edss['demand'] = veg_2_results [2]   
        
        max_total = veg_2_edss['demand'] + veg_1_edss['demand']
        ...
        plt.plot([0,total_demand],[total_demand,0], linestyle='--', label='constraints', alpha=0.6, c='blue')
        # plt.vlines( max_plantable_1, 0, max_plantable_2*1.1, linestyle='--', label='constraints max plants', alpha=0.6)
        # plt.hlines( max_plantable_2, 0, max_plantable_1*1.1, linestyle='--', label='constraints max plants', alpha=0.6)
        plt.plot([0, max_total], [max_total,0], linestyle='--', label='constraints', alpha=0.6, c='cyan')
    
    elif land_veg_1 != None and land_veg_2!=None and too_much==False:
          
        plt.figure(figsize=(10,10))
        max_total = veg_2_edss['demand'] + veg_1_edss['demand']
        # max_plantable_1 = land_veg_1*veg_1_edss['plants_hact']*(veg_1_edss['squeezing_perc'])
        # max_plantable_2 = land_veg_2*veg_2_edss['plants_hact']*(veg_2_edss['squeezing_perc'])
        max_plantable_1 = original_1*1.3
        max_plantable_2 = original_2*1.3
        max_total = max_plantable_2+max_plantable_1
    ...
    # plt.annotate('Feasable area', xy=(total_demand,total_demand))
...
# # Class "S3" is the symbiotic simualtion of the reference object, thus of plants growth process in 4 discrete states
...
# # Demand and loss rate variations are random and can occur at any time during production, however only in "greenhouse"stage the producer can inject more input
# # in the production.
...
class s3():
    ...
    def __init__(self,names, edss, tot_workers, tot_demand):
        self.env = simpy.rt.RealtimeEnvironment(factor=0.0001, strict=False)   #This is not yet real time, adjust to factor = 0 for wall-clock wise simualtion)
...
# Save the original demand because the land bought is based on this, and furher increase in sowing activity will use this value 
    ...
    def farming(self, env, veg, farmer, stage, farmtime):
        ...
        while stage<3:
            print('\ncauli demand', self.veg_1.edss['demand'], 'cauli demand', self.veg_2.edss['demand'], 'total', self.tot_demand)
...
# or it could be constanly uploaded from an excel spreadsheet, but for the sake of demostration of the algorithm it has been implemented with random.choice 
                        ...
                        for i in stochastic_demand:
                            if i == veg.edss['demand']:
                                print('original demand ', veg.edss['demand'])
                                increase = np.random.randint(veg.edss['demand']*0.02,veg.edss['demand']*0.15)
                                veg.edss['demand'] += increase
                                self.tot_demand+=increase
                                print ("slef original demand of veg 1 and 2 are:",self.original_demand_1,self.original_demand_2,"and EDSS demand are:",self.veg_1.edss['demand'],self.veg_2.edss['demand'] )
                                ...
                                # # self.tot_demand+=np.random.randint(self.tot_demand*0.02,self.tot_demand*0.15)   ## IF REMOVE COMMENT HERE, RANDOM TOTAL DEMAND IS ACTIVATED, HOWEVER PLOTS ARE BIASED WHEN THIS VARIABLE IS IN THE MODEL. ONLY NUMERICAL SOLUTIONS ARE CORRECT
...
# If the optimiztion can't find a solutions, then the farmer will automatialready_fully plant
            ...
            # # # if remove comment in the below block, alternative scenario search will be activated
            ...
            if stage>=0 and stage <= 2:
                ...
                actual_production[f"VISIBILE_OVERPRODUCTION (WASTE) for {veg.name} : "] = abs((veg.est_yield*((1-real_loss_rate))) - veg.edss['demand'])
...
name_1 = 'lettuce'
name_2 = 'cauliflower'
edss_lettuce = {'name':name_1.title(), 'upper_loss_rate' : [0.3,0.4,0.4,0.55], 'lower_loss_rate' : [0.2,0.2,0.3,0.4], 'upper_new_loss_rate' : [0.3,0.4,0.5,0.55],'lower_new_loss_rate' : [0.2,0.2,0.3,0.4],
...
edss_cauli = {'name':name_2.title(), 'upper_loss_rate' : [0.2,0.2,0.3,0.4], 'lower_loss_rate' : [0.1,0.1,0.05,0.1],'upper_new_loss_rate' : [0.2,0.3,0.3,0.4], 'lower_new_loss_rate' : [0.1,0.1,0.05,0.1],
...
# initial_demands = {'veg_1':edss_lettuce['demand'], 'veg_2':edss_cauli['demand']} #store initial demands
...
farm.generate()
# plt.show()   # If this is entered in the optimization function before "return", the plots are shown in real time (only on spyder)
